...
export const connectToUser = async (currentUserId, userToConnectId) => {
  await updateDoc(doc(db, "users", userToConnectId), {
    connections: arrayUnion(currentUserId),
  });
  await updateDoc(doc(db, "users", currentUserId), {
    connections: arrayUnion(userToConnectId),
  });
};

export const unConnectToUser = async (currentUserId, userToConnectId) => {
  await updateDoc(doc(db, "users", userToConnectId), {
    connections: arrayRemove(currentUserId),
  });
  await updateDoc(doc(db, "users", currentUserId), {
    connections: arrayRemove(userToConnectId),
  });
};

export const isUsersConnected = async (currentUserId, userToConnectId) => {
  const currentUserDetails = await getUserDetails(currentUserId);
  //console.log(currentUserDetails.connections.includes(userToConnectId));
  return currentUserDetails.connections.includes(userToConnectId);
};
